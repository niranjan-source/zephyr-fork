...
static int lpc84x_uart_poll_in(const struct device *dev, unsigned char *c)
{
	...
	if ((USART_GetStatusFlags(config->base) & kUSART_RxReady) == 0) {
		return -1;
	}
...
static int lpc84x_uart_err_check(const struct device *dev)
{
	...
	USART_ClearStatusFlags(config->base, kUSART_HardwareOverrunFlag | kUSART_ParityErrorFlag |
...
static int lpc84x_uart_fifo_fill(const struct device *dev, const uint8_t *tx_data, int size)
{
	...
	while (tx_size < size && (USART_GetStatusFlags(config->base) & kUSART_TxReady)) {
		USART_WriteByte(config->base, tx_data[tx_size++]);
	}
...
static int lpc84x_uart_fifo_read(const struct device *dev, uint8_t *rx_data, const int size)
{
	...
	while (rx_size < size && (USART_GetStatusFlags(config->base) & kUSART_RxReady)) {
		rx_data[rx_size++] = USART_ReadByte(config->base);
	}
...
static int lpc84x_uart_irq_tx_ready(const struct device *dev)
{
	...
	return (data->irq_status & kUSART_TxReady) &&
	       (USART_GetEnabledInterrupts(config->base) & kUSART_TxReadyInterruptEnable);
}
...
static int lpc84x_uart_irq_rx_ready(const struct device *dev)
{
	...
	return (data->irq_status & kUSART_RxReady) &&
	       (USART_GetEnabledInterrupts(config->base) & kUSART_RxReadyInterruptEnable);
}
...
static int lpc84x_uart_irq_err_ready(const struct device *dev)
{
	...
	return (data->irq_status & (kUSART_HardwareOverrunFlag | kUSART_FramErrorFlag |
	       ...
	       (USART_GetEnabledInterrupts(config->base) & mask);
}
...
static int lpc84x_uart_irq_update(const struct device *dev)
{
	...
	data->irq_status = USART_GetStatusFlags(config->base);
...
static bool lpc84x_uart_is_ready_for_reconfig(const struct device *dev)
{
	const struct lpc84x_uart_config *config = dev->config;
	uint32_t flags;

	/* Read status once */
	flags = USART_GetStatusFlags(config->base);

	/* TX must be fully idle (shift register empty) */
	if ((flags & kUSART_TxIdleFlag) == 0U) {
		return false;
	}

	/* Flush RX FIFO (if any data pending) */
	while (flags & kUSART_RxReady) {
		(void)USART_ReadByte(config->base);
		flags = USART_GetStatusFlags(config->base);
	}
...
static int lpc84x_uart_configure(const struct device *dev, const struct uart_config *cfg)
{
	...
	case UART_CFG_PARITY_NONE:
		data->usart_cfg.parityMode = kUSART_ParityDisabled;
		break;
	case UART_CFG_PARITY_EVEN:
		data->usart_cfg.parityMode = kUSART_ParityEven;
		break;
	case UART_CFG_PARITY_ODD:
		data->usart_cfg.parityMode = kUSART_ParityOdd;
		break;
	default:
		return -ENOTSUP;
	}
	...
	case UART_CFG_STOP_BITS_1:
		data->usart_cfg.stopBitCount = kUSART_OneStopBit;
		break;
	case UART_CFG_STOP_BITS_2:
		data->usart_cfg.stopBitCount = kUSART_TwoStopBit;
		break;
	default:
		return -ENOTSUP;
	}
	...
	case UART_CFG_DATA_BITS_7:
		data->usart_cfg.bitCountPerChar = kUSART_7BitsPerChar;
		break;
	case UART_CFG_DATA_BITS_8:
		data->usart_cfg.bitCountPerChar = kUSART_8BitsPerChar;
		break;
	default:
		return -ENOTSUP;
	}
	...
	case UART_CFG_FLOW_CTRL_NONE:
		data->usart_cfg.enableHardwareFlowControl = false;
		break;
	case UART_CFG_FLOW_CTRL_RTS_CTS:
		data->usart_cfg.enableHardwareFlowControl = true;
		break;
	default:
		return -ENOTSUP;
	}
	...
	data->usart_cfg.baudRate_Bps = cfg->baudrate;

	if (!lpc84x_uart_is_ready_for_reconfig(dev)) {
		return -EBUSY;
	}
	...
	config->base->CFG = USART_CFG_PARITYSEL(data->usart_cfg.parityMode) |
			    USART_CFG_STOPLEN(data->usart_cfg.stopBitCount) |
			    USART_CFG_SYNCEN((uint32_t)data->usart_cfg.syncMode >> 1) |
			    USART_CFG_DATALEN((uint8_t)data->usart_cfg.bitCountPerChar) |
			    USART_CFG_LOOP(data->usart_cfg.loopback ? 1U : 0U) |
			    USART_CFG_SYNCMST(data->usart_cfg.syncMode) |
			    USART_CFG_CLKPOL(data->usart_cfg.clockPolarity) |
			    USART_CFG_CTSEN(data->usart_cfg.enableHardwareFlowControl ? 1U : 0U);

	/* Set baudrate before enabling */
	if (USART_SetBaudRate(config->base, data->usart_cfg.baudRate_Bps, clock_freq) !=
	    kStatus_Success) {
		return -EIO;
	}
	...
	USART_EnableTx(config->base, data->usart_cfg.enableTx);
	USART_EnableRx(config->base, data->usart_cfg.enableRx);
...
static int lpc84x_uart_init(const struct device *dev)
{
	...
	instance = USART_GetInstance(config->base);
	...
	if (USART_SetBaudRate(config->base, data->usart_cfg.baudRate_Bps, clock_freq) !=
		...
		LOG_ERR("failed to set baud rate for USART%d (base %p, rate %u, clock %u)",
			instance, config->base, config->baudrate, clock_freq);
		return -EIO;
	}
